...
export default function DailySunnahPage() {
  const { locale } = useParams<{ locale: string }>();
  const t = useTranslations('dailySunnah');
  ...
  return (
    <div className="min-h-screen bg-cream">
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-amber-500 via-amber-600 to-amber-800 text-white py-16 relative overflow-hidden">
        <motion.div
          ...
          <div className="text-center">
            ...
            <motion.div className="flex justify-center gap-8 mt-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
              <div className="text-center">
                ...
                <p className="text-white/60 text-sm">{locale === 'th' ? 'สุนนะฮ์' : 'Sunnahs'}</p>
              </div>
              <div className="text-center">
                <p className="text-3xl font-bold text-gold">24/7</p>
                <p className="text-white/60 text-sm">{locale === 'th' ? 'ทุกวัน' : 'Daily'}</p>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="py-12 px-4">
        <div className="max-w-4xl mx-auto">
          <JourneyMapNew steps={steps} journeyIcon="☀️" journeyColor="amber" onStepClick={handleStepClick} locale={locale} externalCurrentStep={isPopupOpen ? selectedIndex : undefined} />
        </div>
      </section>
