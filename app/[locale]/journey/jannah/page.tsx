...
export default function JannahPage({ params: { locale } }: { params: { locale: string } }) {
  const t = useTranslations('jannah');
  ...
  return (
    <div className="min-h-screen bg-cream">
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-emerald-500 via-teal-600 to-cyan-700 text-white py-16 relative overflow-hidden">
        {/* Animated Pattern */}
        <motion.div
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="text-center">
            ...
            <motion.p
              className="text-3xl font-arabic text-emerald-200 mb-4"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              <div className="text-center">
                <p className="text-3xl font-bold text-emerald-200">{steps.length}</p>
                <p className="text-white/60 text-sm">{locale === 'th' ? 'หัวข้อ' : 'Topics'}</p>
              </div>
              <div className="text-center">
                <p className="text-3xl font-bold text-emerald-200">8</p>
                <p className="text-white/60 text-sm">{locale === 'th' ? 'ประตู' : 'Gates'}</p>
              </div>
            </motion.div>
          </div>
        </div>
