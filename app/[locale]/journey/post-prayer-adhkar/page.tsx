...
export default function PostPrayerAdhkarPage() {
  const { locale } = useParams<{ locale: string }>();
  const t = useTranslations('postPrayerAdhkar');
  ...
  return (
    <div className="min-h-screen bg-cream">
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-purple-600 via-purple-700 to-purple-900 text-white py-16 relative overflow-hidden">
        <motion.div
          ...
          <div className="text-center">
            ...
            <motion.div className="flex justify-center gap-8 mt-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
              <div className="text-center">
                ...
                <p className="text-white/60 text-sm">{locale === 'th' ? 'อัซการ' : 'Adhkar'}</p>
              </div>
              <div className="text-center">
      ...
      <section className="py-12 px-4">
        <div className="max-w-4xl mx-auto">
          <JourneyMapNew steps={steps} journeyIcon="📿" journeyColor="purple" onStepClick={handleStepClick} locale={locale} externalCurrentStep={isPopupOpen ? selectedIndex : undefined} />
        </div>
      </section>
      ...
      <StepPopup
        ...
        locale={locale}
        journeyType="post-prayer-adhkar"
      />
    </div>
  );
}
