...
export default function MorningEveningAdhkarPage({ params: { locale } }: { params: { locale: string } }) {
  const t = useTranslations('morningEveningAdhkar');
  ...
  return (
    <div className="min-h-screen bg-cream">
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-sky-600 via-sky-700 to-sky-900 text-white py-16 relative overflow-hidden">
        <motion.div
          ...
          <div className="text-center">
            ...
            <motion.div className="flex justify-center gap-8 mt-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
              <div className="text-center">
                ...
                <p className="text-white/60 text-sm">{locale === 'th' ? 'ดุอา' : 'Duas'}</p>
              </div>
              <div className="text-center">
                <p className="text-3xl font-bold text-gold">2x</p>
                <p className="text-white/60 text-sm">{locale === 'th' ? 'ต่อวัน' : 'Daily'}</p>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="py-12 px-4">
        <div className="max-w-4xl mx-auto">
          <JourneyMapNew steps={steps} journeyIcon="🌅" journeyColor="sky" onStepClick={handleStepClick} locale={locale} />
        </div>
      </section>
      ...
      <StepPopup
        ...
        locale={locale}
        journeyType="morning-evening-adhkar"
      />
    </div>
  );
}
